...
<div style="width:100%; max-width:420px;">
    ...
    <div style="text-align:center; margin-bottom:32px;">
        ...
        <h1 style="font-family:Georgia,serif; font-size:1.8rem; font-weight:700; color:#1c1917; margin:0 0 8px;">Forgot Password?</h1>
        <p style="font-size:.88rem; color:#78716c; margin:0;">Enter your email and we'll send you a reset link</p>
    </div>

    {{-- Success --}}
    @if(session('status'))
    <div style="background:#f0fdf4; border:1px solid #86efac; border-radius:8px; padding:12px 16px; margin-bottom:20px;">
        <p style="font-size:.83rem; color:#16a34a; margin:0;">{{ session('status') }}</p>
    </div>
    @endif
    ...
    <div style="background:#fff; border:1px solid #e9e7e1; border-radius:12px; padding:32px;">
        <form method="POST" action="{{ route('password.email') }}">
            ...
            <div style="margin-bottom:24px;">
                <label style="display:block; font-size:.75rem; font-weight:600; color:#78716c; text-transform:uppercase; letter-spacing:.08em; margin-bottom:8px;">Email Address</label>
